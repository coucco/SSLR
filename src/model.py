...
class StructuredController(nn.Module):
    """Оригинальный класс StructuredController из Colab"""
    def __init__(self, cfg, roll_flat_dim, t_frames):
        ...
        self.dims = dict(struct=zs, energy=ze, rhythm=zr, harmony=zh, melody=zm, total=total_z)
        self.cfg = cfg
        self.t = t_frames
    ...
    def forward(self, x_flat):
        ...
        return dict(
            z_mv_hat=z_mv_hat, z_parts=z, enc=enc,
            e_pred=e_pred, r_pred=r_pred, h_pred=h_pred, m_pred=m_pred,
        )

    def sample_structured_z(self, n, device):
        return {k: torch.randn(n, d, device=device) for k, d in self.dims.items() if k != 'total'}
